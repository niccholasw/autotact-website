...
function LogoMark() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true" className="transition-transform duration-300 group-hover:rotate-12">
  ...
  return (
    <header
      className={clsx(
        ...
        scrolled
          ? 'bg-[#121218]/95 backdrop-blur-xl border-b border-rule shadow-[0_1px_12px_rgba(0,0,0,0.4)]'
    ...
    >
      ...
      >
        <div className="bg-[#121218]/98 backdrop-blur-xl px-6 py-5 space-y-1">
          {navLinks.map(({ label, href }, i) => (
            <Link
              key={href}
              href={href}
              ...
              className={clsx(
                ...
                pathname === href ? 'text-ink font-medium bg-white/5' : 'text-ink-3 hover:text-ink hover:bg-white/[0.03]'
              )}
